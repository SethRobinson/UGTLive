...
set "TEST_IMAGE=%SCRIPT_DIR%..\shared\test_images\english_manga.png"
...
curl -s -X POST http://localhost:!PORT!/process?lang=english -H "Content-Type: application/octet-stream" --data-binary "@%TEST_IMAGE%"
echo.
...
for /L %%i in (1,1,5) do (
    echo   Request %%i/5...
    curl -s -X POST http://localhost:!PORT!/process?lang=english -H "Content-Type: application/octet-stream" --data-binary "@%TEST_IMAGE%" -w "    Time: %%{time_total}s\n" -o nul
)
...
echo   curl -X POST http://localhost:!PORT!/process?lang=english \
