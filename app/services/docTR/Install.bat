...
echo [Step 1/9] Upgrading pip and installing core dependencies first...
echo [Step 1/9] Upgrading pip and core deps... >> "%LOG_FILE%"
python -m pip install --upgrade pip setuptools wheel >> "%LOG_FILE%" 2>&1
...
echo [Step 2/9] Installing PyTorch nightly with CUDA 12.8 support...
echo This may take several minutes...
echo [Step 2/9] Installing PyTorch nightly (CUDA 12.8)... >> "%LOG_FILE%"
...
echo [Step 3/9] Installing docTR dependencies...
echo [Step 3/9] Installing docTR dependencies... >> "%LOG_FILE%"
...
echo [Step 4/9] Installing python-doctr with dependencies...
echo [Step 4/9] Installing python-doctr... >> "%LOG_FILE%"
REM Install python-doctr with all deps - it may downgrade torch but we'll fix that
python -m pip install "python-doctr[torch]" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo ERROR: Failed to install python-doctr!
    echo ERROR: Failed to install python-doctr >> "%LOG_FILE%"
    pause
...
echo [Step 5/9] Reinstalling PyTorch nightly to ensure correct version...
echo [Step 5/9] Reinstalling PyTorch nightly... >> "%LOG_FILE%"
...
echo [Step 6/9] Skipping - merged into step 4...
echo [Step 6/9] Skipped >> "%LOG_FILE%"

echo [Step 7/9] Installing FastAPI and Uvicorn...
echo [Step 7/9] Installing FastAPI/Uvicorn... >> "%LOG_FILE%"
...
echo [Step 8/9] Installing Scikit-learn (optional but useful for color analysis fallback)...
echo [Step 8/9] Installing Scikit-learn... >> "%LOG_FILE%"
python -m pip install scikit-learn >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo WARNING: Scikit-learn installation failed, some fallbacks may be limited
    echo WARNING: Scikit-learn installation failed >> "%LOG_FILE%"
...
echo [Step 9/9] Initializing docTR (first-run warmup)...
echo [Step 9/9] Initializing docTR... >> "%LOG_FILE%"
REM Set SSL cert for model downloads (certifi is now installed)
for /f "delims=" %%i in ('python -c "import certifi; print(certifi.where())"') do set "SSL_CERT_FILE=%%i"
python -c "import ssl, certifi; ssl._create_default_https_context = lambda: ssl.create_default_context(cafile=certifi.where()); from doctr.models import ocr_predictor; ocr_predictor(det_arch='db_resnet50', reco_arch='master', pretrained=True)" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
